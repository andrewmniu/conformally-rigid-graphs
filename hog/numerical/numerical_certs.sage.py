...
# This file was *autogenerated* from the file numerical_certs.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_150 = Integer(150); _sage_const_120 = Integer(120); _sage_const_100 = Integer(100); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_1en8 = RealNumber('1e-8'); _sage_const_3600 = Integer(3600); _sage_const_60 = Integer(60); _sage_const_6 = Integer(6); _sage_const_100p0 = RealNumber('100.0')
import csv
import json
...
sys.path.insert(_sage_const_0 , os.path.abspath('../cr'))
from solvers import (
...
def _graph_task(g6, threshold):
    ...
    if n < _sage_const_2  or not G.is_connected():
        return None

    L_np = np.asarray(
        G.laplacian_matrix().change_ring(RDF).numpy(), dtype=float
    )
    eigvals = np.sort(np.linalg.eigh(L_np)[_sage_const_0 ])
    l2_val  = float(eigvals[_sage_const_1 ])
    ln_val  = float(eigvals[-_sage_const_1 ])
    l2_mult = int(np.sum(np.isclose(eigvals, l2_val, atol=_sage_const_1en8 )))
    ln_mult = int(np.sum(np.isclose(eigvals, ln_val, atol=_sage_const_1en8 )))
...
def _fmt_time(seconds):
    seconds = int(seconds)
    h, rem = divmod(seconds, _sage_const_3600 )
    m, s = divmod(rem, _sage_const_60 )
    if h:
...
already_done = set()
resume_mode = os.path.exists(OUTPUT_CSV) and os.path.getsize(OUTPUT_CSV) > _sage_const_0 
if resume_mode:
...
with open(DATA_PATH) as _f:
    n_total = sum(_sage_const_1  for _ in _f)
...
def _new_pool():
    return _mp_ctx.Pool(processes=_sage_const_1 )

with open(DATA_PATH) as f_in,      open(OUTPUT_CSV, file_mode, newline='') as f_out,      open(TIMED_OUT_FILE, 'a') as f_timeout:
    ...
    pool = _new_pool()
    total = cr_count = error_count = timeout_count = rows_since_flush = _sage_const_0 
    t_start = time.time()
    ...
    for raw in f_in:
            ...
            if result is None:
                ...
                if result['is_lcr'] or result['is_ucr']:
                    writer.writerow({
                        ...
                        "lambda_2":       round(result['l2_val'], _sage_const_6 ),
                        "lambda_2_mult":  result['l2_mult'],
                        "lcr":            result['is_lcr'],
                        "ucr":            result['is_ucr'],
                        "lambda_n":       round(result['ln_val'], _sage_const_6 ),
                    ...
                    cr_count += _sage_const_1 
                    rows_since_flush += _sage_const_1 
                    if rows_since_flush >= FLUSH_EVERY:
                        f_out.flush()
                        rows_since_flush = _sage_const_0 

        except mp.TimeoutError:
            timeout_count += _sage_const_1 
            pool.terminate()
            pool.join()
            pool = _new_pool()
        ...
        rate    = total / elapsed if elapsed > _sage_const_0  else _sage_const_0 
        eta     = _fmt_time((n_total - total) / rate) if rate > _sage_const_0  else "?"
        n_str   = str(n) if n is not None else "?"
        sys.stdout.write(
            f"\r[{total:>6}/{n_total}] {_sage_const_100p0 *total/n_total:5.1f}%"
