...
# This file was *autogenerated* from the file exact_certs.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_300 = Integer(300); _sage_const_20 = Integer(20); _sage_const_1p0 = RealNumber('1.0'); _sage_const_3600 = Integer(3600); _sage_const_60 = Integer(60); _sage_const_1 = Integer(1); _sage_const_100p0 = RealNumber('100.0')
import csv
import json
...
CERT_TIMEOUT = _sage_const_300 
FLUSH_EVERY  = _sage_const_20 
...
def _structural_cert(row):
    ...
    edge_trans = (eo is not None and eo == _sage_const_1p0 )
    ...
    reg_bip    = (reg == _sage_const_1p0  and bip == _sage_const_1p0 )
...
total = len(all_rows)
done = exact_count = struct_count = infeas_count = mult_count = timeout_count = error_count = _sage_const_0 
t_start = time.time()

with open(OUTPUT_CSV,       file_mode, newline='') as f_csv,       open(PHI_JSONL,        'a')                   as f_phi,       open(INFEASIBLE_JSONL, 'a')                   as f_infeas,      open(MULT_ISSUES_JSONL,'a')                   as f_mult,       open(TIMEOUT_JSONL,    'a')                   as f_timeout:
    ...
    for row in all_rows:
        ...
        rate = done / elapsed if elapsed > _sage_const_0  else _sage_const_0 
        eta  = _fmt_time((total - done) / rate) if rate > _sage_const_0  else "?"
        sys.stdout.write(
            f"\r[{done:>6}/{total}] {_sage_const_100p0 *done/total:5.1f}%"
